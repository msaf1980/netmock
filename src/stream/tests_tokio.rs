...
#[cfg(feature = "tokio")]
#[tokio::test]
async fn simple_mockstream_writing() {
...
#[cfg(feature = "tokio")]
#[tokio::test]
async fn simple_mockstream_reading() {
    let read = b"Hello\nGoodbye\n";
    let mut stream = SimpleMockStream::new(read.to_vec());
    let mut buf = Vec::<u8>::with_capacity(20);
    let readed = stream.read_to_end(&mut buf).await.unwrap();
    assert_eq!(&buf, read);
...
async fn simple_mockstream() {
    ...
    stream.write_all(b"WRITE").await.unwrap();
...
async fn checked_mockstream() {
    ...
    let result = stream.write_all(b"Missmatch").await;
    assert!(result.is_err());
    assert_eq!(stream.written(), &vec![]);
...
#[cfg(feature = "tokio")]
#[tokio::test]
async fn checked_mockstream_error() {
